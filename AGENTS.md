...
| `kimi` | `kimi-cli` (thin wrapper over `kimi -p --output-format stream-json`, `packages/ai/providers/kimi-cli.ts`; no models list — `-m` aliases can't be enumerated — and no permission cards, since print mode forces auto permissions) |
