...
| `PLANNOTATOR_ORIGIN` | Explicit agent-origin override at the top of the detection chain. Valid values: `claude-code`, `amp`, `droid`, `opencode`, `codex`, `copilot-cli`, `gemini-cli`, `kiro-cli`, `pi`, `oh-my-pi`, `kimi`. Invalid values silently fall through to env-based detection. Unset by default. |
...
| `kimi` | `kimi-cli` (drives a long-lived `kimi acp` agent over ACP — newline-delimited JSON-RPC 2.0 on stdio, `packages/ai/providers/kimi-cli.ts`; token-level streaming, thinking stream, permission cards via `session/request_permission`, model list from `session/new` configOptions, native fork/resume; only limitation: no cost statistics, since the kimi engine exposes no pricing data) |
...
> **Kimi transport note:** the `kimi-cli` provider no longer shells out to one-shot `kimi -p --output-format stream-json` runs. It drives a long-lived `kimi acp` process over ACP (`packages/ai/providers/kimi-cli.ts`), advertising no client capabilities at `initialize` so the agent does its own IO and routes approvals through `session/request_permission` (the existing PermissionCard UI answers them). Sessions are switched to kimi's `auto` permission mode at start (`session/set_config_option` with `configId: "mode"`, best-effort), so read-only tools run without prompting while kimi's own policies still gate dangerous operations. Model discovery is deferred behind the provider initializer (`?activate=` from the model picker, or the first kimi session) exactly like Codex — nothing spawns at server boot, so the picker lists kimi with an empty model list until first activation. A kimi too old to provide `kimi acp` fails the handshake with an upgrade hint.
