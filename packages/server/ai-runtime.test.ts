...
describe("createAIRuntime Kimi discovery", () => {
  ...
  test("capabilities?activate= runs kimi model discovery once and shares it with the session path", async () => {
    if (process.platform === "win32") return;

    const dir = mkdtempSync(join(tmpdir(), "plannotator-activate-kimi-"));
    tempDirs.push(dir);
    const marker = join(dir, "kimi-ran");
    const kimi = join(dir, "kimi");
    writeFileSync(kimi, `#!/bin/sh\necho ran >> '${marker}'\nexit 1\n`);
    ...
    writeFileSync(runner, `
      ...
      const runsAfterProbe = runs();
      const activate = await runtime.endpoints["/api/ai/capabilities"](
        new Request("http://localhost/api/ai/capabilities?activate=kimi-cli"),
      );
      const runsAfterActivate = runs();
      const session = await runtime.endpoints["/api/ai/session"](
        new Request("http://localhost/api/ai/session", {
